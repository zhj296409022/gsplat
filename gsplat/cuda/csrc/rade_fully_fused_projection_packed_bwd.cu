...
__global__ void fully_fused_projection_packed_bwd_kernel(
    ...
    vec3<T> v_mean_c(0.f);
    rade_persp_proj_vjp<T>(mean_c, covar_c, fx, fy, cx, cy, image_width, image_height,
...
rade_fully_fused_projection_packed_bwd_tensor(
    ...
    if (nnz) {
        fully_fused_projection_packed_bwd_kernel<float><<<(nnz + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS, GSPLAT_N_THREADS, 0, stream>>>(
