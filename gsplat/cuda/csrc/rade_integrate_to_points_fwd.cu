...
__global__ void integrate_to_points_fwd_kernel(
    ...
    const vec3<S> *__restrict__ scales,  // [C, N, 3]
    ...
    const S *__restrict__ ray_ts,  // [C, N]
    const S *__restrict__ invraycov3Ds,  // [C, N, 6]
    const bool *__restrict__ conditions, // [C, N]
    ...
    S *__restrict__ render_colors, // [C, image_height, image_width, COLOR_DIM + 1]
    S *__restrict__ render_alphas, // [C, image_height, image_width, 1]
    S *__restrict__ out_integrated_alphas, // [C, PN]
    S *__restrict__ out_integrated_colors, // [C, PN, COLOR_DIM]
    S *__restrict__ out_coordinate2d, // [C, PN, 2]
    S *__restrict__ out_sdf, // [C, PN, 6]
    ...
    out_coordinate2d += camera_id * PN * 2;
    out_sdf += camera_id * PN * 6;
    ...
    vec3<S> *depth_depth_plane_batch = 
        reinterpret_cast<vec3<float> *>(&points2d_depth_batch[block_size]); // [block_size]
    vec3<S> *scale_batch = 
        reinterpret_cast<vec3<float> *>(&depth_depth_plane_batch[block_size]); // [block_size]
	...
	S mid_depth_center = 0;
	vec2<S> mid_plane = {0, 0};
	vec2<S> mid_mean2d = {0, 0};
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        if (idx < range_end) {
            ...
            depth_depth_plane_batch[tr] = { ray_ts[g], ray_planes[g].x, ray_planes[g].y };
        }
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            const S depth_center = depth_depth_plane_batch[t].x;
            const vec2<S> depth_plane = {depth_depth_plane_batch[t].y, depth_depth_plane_batch[t].z};
            const vec2<S> xy = {xy_opac.x, xy_opac.y};
            bool used = false;
            for (uint32_t k = 0; k < 5; ++k){
                const vec3<S> ray_k = {(px + offset_xs[k] - cx) / focal_x, (py +offset_ys[k] - cy) / focal_y, 1.0};

                S depth = depth_center + (depth_plane.x * ray_k.x + depth_plane.y * ray_k.y);

                S power = -0.5f * (conic.x * ray_k.x * ray_k.x + conic.z * ray_k.y * ray_k.y) - conic.y * ray_k.x * ray_k.y;
                if (power > 0.0f) {
                    continue;
                }
    ...
    while (true){
        ...
        vec3<S> ray_point = { (px - image_width/2.) / focal_x, (py - image_height/2.) / focal_y, 1.0 };
        S depth2distance = sqrt(ray_point.x*ray_point.x+ray_point.y*ray_point.y+ray_point.z*ray_point.z);
        ...
        if (inside) {
            render_colors[pix_id * (COLOR_DIM + 1 + 3) + 3] = (float)total_projected;
        }
    }
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> call_kernel_with_dim(
    ...
    const torch::Tensor &scales,    // [C, N, 3] or [nnz, 3]
    ...
    const torch::Tensor &invraycov3Ds,  // [C, N, 6]
    const torch::Tensor &conditions, // [C, N]
    ...
    GSPLAT_CHECK_INPUT(view2gaussians);
    GSPLAT_CHECK_INPUT(ray_planes);
    GSPLAT_CHECK_INPUT(ray_ts);
    GSPLAT_CHECK_INPUT(Ks);
    GSPLAT_CHECK_INPUT(tile_offsets);
    GSPLAT_CHECK_INPUT(flatten_ids);
    GSPLAT_CHECK_INPUT(points2d);
    GSPLAT_CHECK_INPUT(point_depths);
    GSPLAT_CHECK_INPUT(point_tile_offsets);
    GSPLAT_CHECK_INPUT(point_flatten_ids);
    ...
    torch::Tensor out_integrated_colors = torch::full({C, PN, CDIM}, 0.0,
                                        means2d.options().dtype(torch::kFloat32));
    torch::Tensor out_coordinate2d = torch::full({C, PN, 2}, 0.0, 
                                        means2d.options().dtype(torch::kFloat32));
    torch::Tensor out_sdf = torch::full({C, PN, 6}, 0.0, 
                                        means2d.options().dtype(torch::kFloat32));
    ...
    const uint32_t shared_mem =
        ...
        sizeof(vec10<float>) + sizeof(vec3<float>));
    ...
    integrate_to_points_fwd_kernel<CDIM, float>
        <<<blocks, threads, shared_mem, stream>>>(
            ...
            out_coordinate2d.data_ptr<float>(),
            out_sdf.data_ptr<float>(),
            last_ids.data_ptr<int32_t>());

    return std::make_tuple(renders, out_integrated_alphas, out_integrated_colors, out_coordinate2d, out_sdf);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> rade_integrate_to_points_fwd_tensor(
    // Point parameters
    const torch::Tensor &points2d,                   // [C, N, 2]
    const torch::Tensor &point_depths,                    // [C, N, 3]
    // Gaussian parameters
...
#define __GS__CALL_(N)                                                                 \
    case N:                                                                            \
        return call_kernel_with_dim<N>(points2d, point_depths,                         \
                                       means2d, scales, conics, colors, opacities,             \
                                       view2gaussians, ray_planes, ray_ts,             \
                                       invraycov3Ds, conditions, Ks,                   \
