...
view_to_gaussians_bwd_tensor(
    ...
    GSPLAT_CHECK_INPUT(v_view2gaussians);
    ...
    if (C && N) {
        view_to_gaussians_bwd_kernel<float><<<(C * N + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS, GSPLAT_N_THREADS, 0, stream>>>(
            C, N, 
        ...
        // view_to_gaussians_bwd_kernel<double><<<(C * N + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS, GSPLAT_N_THREADS, 0, stream>>>(
        //     C, N, 
