...
torch::Tensor view_to_gaussians_fwd_tensor(
    ...
    if (C && N) {
        view_to_gaussians_fwd_kernel<float><<<(C * N + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS, GSPLAT_N_THREADS, 0, stream>>>(
        ...
        // view_to_gaussians_fwd_kernel<double><<<(C * N + GSPLAT_N_THREADS - 1) / GSPLAT_N_THREADS, GSPLAT_N_THREADS, 0, stream>>>(
